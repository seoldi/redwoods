	...
	<div class="r-sub-con">
		...
		<div id="bo_list" style="width:<?php echo $width; ?>">
				...
				<fieldset id="bo_sch">
						...
						<input type="button" class="btn_b02 btn" value="Excel" onclick="ReportToExcelConverter()">
						</form>
				</fieldset>
				<!-- } 게시판 검색 끝 -->
				</div>
			...
			<table id="table2excel">
				<tr class="head">
					...
					<?php } ?>
					<th class="s-number">번호</th>
					<th class="s-name">성명</th>
					<th class="s-tel">연락처</th>
					<th class="s-location">거주지</th>
					<th class="s-type">관심주택형</th>
					<th class="s-date">등록일</th>
					<!-- <span class="s-view"><?php echo subject_sort_link('wr_hit', $qstr2, 1) ?>조회</a></span>
					<?php if ($is_good) { ?><span class="w50"><?php echo subject_sort_link('wr_good', $qstr2, 1) ?>추천 <i class="fa fa-sort" aria-hidden="true"></i></a></span><?php } ?>
					<?php if ($is_nogood) { ?><span class="w50"><?php echo subject_sort_link('wr_nogood', $qstr2, 1) ?>비추천 <i class="fa fa-sort" aria-hidden="true"></i></a></span><?php } ?>
				...
				<tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
								...
								<td class="s-name" onclick="location.href='<?php echo $list[$i]['href'] ?>'"><?php echo $list[$i]['name'] ?></td>
								<td class="s-tel"><?php echo $list[$i]['wr_1']; ?></td>
								<td class="s-location"><?php echo $ex_addr1; ?></td>
								<td class="s-type"><?php echo $list[$i]['wr_4'] ?></td>
								<td class="s-date"><nobr class="pc_none"></nobr> <?php echo $list[$i]['datetime'] ?></td>
						<?php } ?>
						<?php if (count($list) == 0) { echo '<tr style="text-align:center;">게시물이 없습니다.</tr>'; } ?>
				</tr>
			</table>
				...
				<div class="bo_fx">
						...
						<ul class="btn_bo_user">
								...
								<?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02 btn">글쓰기</a></li><?php } ?>
								<li><input type="button" class="btn_b02 btn" value="Excel" onclick="ReportToExcelConverter()"></li>
						</ul>
						<?php } ?>
				</div>
				<?php } ?>
